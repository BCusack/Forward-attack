...
def CheckHashList(input_start, store_list, input_hash):
    if input_hash in store_list:
        for i, item in enumerate(store_list):
            if item == input_hash:
                print("Alice's number is: " + str(input_start + i))
                break
    else:
        print("Alice's number is not in the list")
...
def run(createStore):
    ...
    input_number = int(input("Enter Alice's Number To Be Hashed: "))
    store_list = createStore(input_start, input_end)
    print('Copy and paste the hash of the number or any hash you want to check')
    print(hashlib.sha256(str(input_number).encode('utf-8')).hexdigest())
    input_hash = str(input("Enter Alice's Hash: ")).lower()
